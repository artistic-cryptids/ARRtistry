...
contract ArtifactApplication {
  ...
  function getProposal(uint proposalId) public view returns (address, address, string memory, string memory, string memory, string memory, string memory, string memory, string memory) {
    ...
    return (who, artifact.artist, artifact._title, artifact._artistNationality, artifact._artistBirthYear, artifact._created, artifact._medium, artifact._size, artifact._metaUri);
  }
}
